...
namespace DevDives\Boilerplate;
...
class WP_Theme_Boilerplate {
	...
	public static function block( $block, $callback_function ) {
		list( $ed_block_id, $ed_block_fields,$ed_option_fields ) = DevDives::defaults( $block['id'] );

		// Set the block name for it's ID & class from it's file name.
		$ed_block_name   = $block['name'];
		$ed_block_name   = str_replace( 'acf/', '', $ed_block_name );
		$ed_block_styles = DevDives::convert_to_css( $block );
		...
		if ( $post_type ) {
			...
			if ( $query->have_posts() ) {
				while ( $query->have_posts() ) {
					...
					list( $ed_var_post_id, $ed_fields, $ed_option_fields ) = DevDives::defaults( $ed_var_post_id );
					// Include specific template for the content.
					if ( isset( $template ) ) {
			...
			if ( $query->have_posts() && $query->max_num_pages > 1 && $pagination ) {
				...
				DevDives::$pagination_fun( $query->max_num_pages );
				echo '</div>';
			}
			return $query;
		}
...
class_alias( 'DevDives\Boilerplate\WP_Theme_Boilerplate', 'Boilerplate' );
