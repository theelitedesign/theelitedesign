...
$ed_var_post_title = $ed_fields['bst_var_post_title'] ?? get_the_title();
...
<div class="container-980">
	<div class="wrapper">
		...
		<div class="post-meta d-flex justify-content-between align-items-center">
			<!-- /.post-tags -->
			<?php if ( $ed_var_post_categories ) { ?>
				<div class="post-cat">
					<?php foreach ( $ed_var_post_categories as $ed_var_category ) { ?>
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class( 'post-ctn' ); ?>>
			...
			if ( 'random' === $ed_var_rp_selection_criteria ) {
				...
				<h3><?php esc_html_e( 'Related Posts', 'elitedesigns_td' ); ?></h3>
							<?php
							foreach ( $ed_var_selected_posts as $ed_var_post_id ) {

								$ed_post_fields = get_fields( $ed_var_post_id );
								if ( ! has_post_thumbnail( $ed_var_post_id ) ) {
									echo '<img class="" src="' . esc_url( get_template_directory_uri() ) . '/assets/build/images/admin/defaults/default-image.webp" >';
								} else {
									echo get_the_post_thumbnail(
